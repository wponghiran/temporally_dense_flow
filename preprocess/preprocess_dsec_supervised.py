...
def preprocess_dataset(events, rect_map, flow_paths, env_name, flow_idx_2_event_idx, flow_ts, event_ts, dt, n_split,
                       save_dir, chunk_idx):
    ...
    for flow_idx, flow_path in enumerate(tqdm(flow_paths, desc='Pre-processing {}'.format(env_name))):
        ...
        sel_xs = torch.from_numpy(sel_rect_xs.astype('float32'))
        sel_ys = torch.from_numpy(sel_rect_ys.astype('float32'))
        ...
        for xlim in [x0, x0+1]:
            for ylim in [y0, y0+1]:
                ...
                index = 640 * 480 * sel_ts_norm.long() + 640 * ylim.long() + xlim.long()
...
if __name__ == '__main__':
    ...
    for env_name, event_data_file_path, \
        ...
        for chunk_idx, (chopped_flow_rts, chopped_flow_paths) in enumerate(
            ...
            preprocess_dataset(events=events, rect_map=rect_map, flow_paths=chopped_flow_paths, env_name=env_name,
